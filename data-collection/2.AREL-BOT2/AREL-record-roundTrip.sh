...
# go to open position
python3 /CARE-U/python-host/switchbot_py3_AREL.py -d $DEVICE -c close >> ${DIR}/errors.log 2>&1
echo opening2 - $(date +'%Y-%m-%d-%H%M%S') >> $LOG

# record for 60 seconds
python3 /CARE-U/depthai-python/examples/encoding_max_limit-AREL.py $DIR opening2 $REC_TIME >> ${DIR}/errors.log 2>&1

# go to closing position
python3 /CARE-U/python-host/switchbot_py3_AREL.py -d $DEVICE -c open >> ${DIR}/errors.log 2>&1
echo closing2 - $(date +'%Y-%m-%d-%H%M%S') >> $LOG

# record for 60 seconds
python3 /CARE-U/depthai-python/examples/encoding_max_limit-AREL.py $DIR closing2 $REC_TIME >> ${DIR}/errors.log 2>&1
